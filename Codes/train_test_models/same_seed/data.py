...
# the rotations and flipings are done when calling the data. It will also use the
...
class make_dataset_multifield2_errors():
    ...
    def __init__(self, mode, seed, f_images, f_params, splits, f_images_norm, 
        ...
        # get the model
        model_m = architecture.get_architecture(arch, hidden, dr, channels)
        model_m.to(device=device)
        network_total_params = sum(p.numel() for p in model_m.parameters())
        ...
        # define th array containing the errors
        ...
        data     = np.zeros((total_maps, channels, height, width), dtype=np.float32)
        ...
        with torch.no_grad():
            for i in range(iterations):
                ...
                inp = torch.tensor(data[start:end,:,:,:]).to(device)
                y_m = model_m(inp).cpu().numpy()
                error[start:end,:] = (y_m - params[start:end,:])**2
            inp = torch.tensor(data[end:,:,:,:]).to(device)
            y_m = model_m(inp).cpu().numpy()
            error[end:,:] = (y_m - params[end:,:])**2

        # normalize the error
        error_m = np.mean(error, axis=0)
        error_s = np.std(error,  axis=0)
        error   = (error - error_m)/error_s

        # keep only the value of the considered mode
        ...
        self.e    = torch.tensor(error,  dtype=torch.float32)
        del data, data_c
    ...
    def __getitem__(self, idx):
        ...
        return maps, self.y[idx], self.e[idx]
...
class make_dataset_Gaussian():
    ...
    def __init__(self, mode, seed, f_images, f_params, splits, f_images_norm, 
        ...
        if params_sims.ndim==1:  params_sims = np.expand_dims(params_sims, axis=1)
        total_sims, total_maps, num_params = \
                params_sims.shape[0], params_sims.shape[0]*splits, params_sims.shape[1]
        params_maps = np.zeros((total_maps, num_params), dtype=np.float32)
        #print(params_sims.shape)
        #sys.exit()
        for i in range(total_sims):
            for j in range(splits):
                params_maps[i*splits + j] = params_sims[i]
        ...
        for channel, (fim, fnorm) in enumerate(zip(f_images, f_images_norm)):
            ...
            if data_c.shape[0]!=total_maps:  
                print('hello1 :',data_c.shape[0])
                print('hello2 :', total_maps)
...
def create_dataset_multifield(mode, seed, f_images, f_params, batch_size, splits, 
    ...
    if Gaussian:
        data_set = make_dataset_Gaussian(mode, seed, f_images, f_params, splits, 
