...
snap3   = '/mnt/ceph/users/lgarrison/CAMELS/CV_G_0/z0.000.asdf'
grid    = 512
MAS     = 'CIC'
axis    = 0
threads = 1
...
np.savetxt('Pk_Gadget.txt', np.transpose([Pk.k3D, Pk.Pk[:,0]]))
...
# compute power spectrum
Pk = PKL.Pk(delta, BoxSize, axis, MAS=None, threads=threads, verbose=True)
np.savetxt('Pk_Ramses.txt', np.transpose([Pk.k3D, Pk.Pk[:,0]]))
...
# compute power spectrum
Pk = PKL.Pk(delta, BoxSize, axis, MAS=None, threads=threads, verbose=True)
np.savetxt('Pk_Abacus.txt', np.transpose([Pk.k3D, Pk.Pk[:,0]]))
