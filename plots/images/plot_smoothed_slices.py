...
sim   = 'Gadget'
f_out = '%s_smoothed_slice.pdf'%sim
...
df2 = scipy.ndimage.gaussian_filter(df1, 3, mode='wrap')
df3 = scipy.ndimage.gaussian_filter(df1, 5, mode='wrap')

print('%.3e < df < %.3e'%(np.min(df1), np.max(df1)))
print('%.3e < df < %.3e'%(np.min(df2), np.max(df2)))
print('%.3e < df < %.3e'%(np.min(df3), np.max(df3)))
min_density, max_density = 1e10, 7e14

ax1.imshow(df1,cmap=get_cmap('nipy_spectral'),origin='lower',
           interpolation='bicubic',
           norm = LogNorm(vmin=min_density,vmax=max_density))

ax2.imshow(df2,cmap=get_cmap('nipy_spectral'),origin='lower',
           interpolation='bicubic',
           norm = LogNorm(vmin=min_density,vmax=max_density))

cax = ax3.imshow(df3,cmap=get_cmap('nipy_spectral'),origin='lower',
                 interpolation='bicubic',
                 norm = LogNorm(vmin=min_density,vmax=max_density))

ax4 = axes([0.92,0.28,0.02,0.43])

cbar = fig.colorbar(cax, ax4) #in ax2 colorbar of ax1
...
ax1.set_title(r"${\rm No\,\,smoothing}$",position=(0.5,1.02),size=18)
ax2.set_title(r"${\rm Smoothing=3}$",position=(0.5,1.02),size=18)
ax3.set_title(r"${\rm Smoothing=5}$",position=(0.5,1.02),size=18)
