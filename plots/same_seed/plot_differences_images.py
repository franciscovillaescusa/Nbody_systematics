...
gs.update(hspace=0.1,wspace=0.1,bottom=0.6,top=1.05)
...
f_out='Comparison_Nbody.pdf'

f1 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_Abacus_fiducial_z=0.00.npy'
f2 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_CUBEP3M_fiducial_z=0.00.npy'
f3 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_Enzo3_fiducial_z=0.00.npy'
f4 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_Gadget_fiducial_z=0.00.npy'
f5 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_PKDGrav_fiducial_z=0.00.npy'
f6 = '/mnt/ceph/users/fvillaescusa/Nbody_systematics/data/maps/same_seed/Images_M_Ramses_fiducial_z=0.00.npy'
...
print('%.3e %.3e'%(np.min(data1), np.max(data1)))
min_density, max_density = 3e10, 3e14

for data, ax in zip([data1,data2,data3,data4,data5,data6],[ax1,ax2,ax3,ax4,ax5,ax6]):
    cax = ax.imshow(data,cmap=get_cmap('rainbow'),origin='lower',
                    interpolation='bicubic',
                    norm = LogNorm(vmin=min_density,vmax=max_density))

ax12=axes([0.13,0.62,0.01,0.17])
cbar = fig.colorbar(cax, ax12) #in ax2 colorbar of ax1


data = (data3-data1)#/data1
print('%.3e %.3e'%(np.min(data), np.max(data)))

min_density, max_density = -1e14, 1e14
for data, ax in zip([data2,data3,data4,data5,data6],[ax7,ax8,ax9,ax10,ax11]):
    cax = ax.imshow((data-data1),cmap=get_cmap('seismic'),origin='lower',
                    interpolation='bicubic', 
                    norm = SymLogNorm(vmin=min_density,vmax=max_density,
                                      linthresh=1e9, linscale=1e-5))

ax13=axes([0.20,0.62,0.01,0.17])
cbar = fig.colorbar(cax, ax13) #in ax2 colorbar of ax1
#cbar.ax.tick_params(labelsize=10)  #to change size of ticks
...
ax1.set_title(r'${\rm Abacus}$',position=(0.5,1.02),size=14)
ax2.set_title(r'${\rm CUBEP3M}$',position=(0.5,1.02),size=14)
ax3.set_title(r'${\rm Enzo}$',position=(0.5,1.02),size=14)
ax4.set_title(r'${\rm Gadget}$',position=(0.5,1.02),size=14)
ax5.set_title(r'${\rm PKDGrav}$',position=(0.5,1.02),size=14)
ax6.set_title(r'${\rm Ramses}$',position=(0.5,1.02),size=14)
